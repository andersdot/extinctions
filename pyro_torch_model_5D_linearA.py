...
class Objective(nn.Module):
    ...
    def predicted_color(self, A, c, coeff):
        return c + A*torch.tensor(coeff)

    def predicted_magnitude(self, A, M, d, coeff):
        return M + A*torch.tensor(coeff) + 5.*torch.log10(d*1e3/10.)
...
def optimize_model(theta_0, chat, mhat, varpihat, sigmac, sigmam, sigmavarpi, dustco_c, dustco_m, ss, i):
    ...
    hes2 = hessian(objective.logjoint(), objective.parameters()) #, create_graph=True)
