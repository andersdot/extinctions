...
def plt_flow_density(prior_logdensity, inverse_transform, ax, npts=100, memory=100, title="$q(x)$", device="cpu"):
    ...
    for ii in torch.split(inds, int(memory**2)):
        print(x[ii].shape, zeros[ii].shape)
...
def plt_flow_samples(prior_sample, transform, ax, npts=100, memory=100, title="$x ~ q(x)$", device="cpu"):
    z = prior_sample(npts * npts, 5).type(torch.float32).to(device)
    zk = []
    ...
    corner.corner(zk)
    #ax.hist2d(zk[:, 0], zk[:, 1], range=[[LOW, HIGH], [LOW, HIGH]], bins=npts, norm=matplotlib.colors.LogNorm())
    #ax.invert_yaxis()
    #ax.get_xaxis().set_ticks([])
    #ax.get_yaxis().set_ticks([])
    #ax.set_title(title)
...
def plt_samples(samples, ax, npts=100, title="$x ~ p(x)$"):
    ax.hist2d(samples[:, 0], samples[:, 1], range=[[LOW, HIGH], [LOW, HIGH]], bins=npts, norm=matplotlib.colors.LogNorm())
    ax.invert_yaxis()
    #ax.get_xaxis().set_ticks([])
    #ax.get_yaxis().set_ticks([])
    ax.set_title(title)
